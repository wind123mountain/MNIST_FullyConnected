...
void train_step(Network_Model &model, int epochs, int batch_size, float lr,
  ...
  for (int epoch = 0; epoch < epochs; epoch++) {
    ...
    for (int step = 0; step < n_batch; step++) {
      float *start_X = train.X + step * batch_size * model.input_size;
      int *start_y = train.y + step * batch_size;
      int n_sample = step < n_batch - 1 ? batch_size : n_sample_last_batch;

      MatrixXf X = Map<MatrixXfRow>(start_X, n_sample, model.input_size);
      VectorXi y = Map<VectorXi>(start_y, n_sample);
      ...
      total_loss += model.loss.call(model.layers[out_layer_id].output, y);
      VectorXi predict = model.to_label(model.layers[out_layer_id].output);
      acc += model.get_num_predict_true(predict, y);
    }
...
int main(int argc, char *argv[]) {
  ...
  train_step(model, epochs, batch_size, lr, train);
